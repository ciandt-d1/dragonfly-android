...
public class ImageUtils {
    ...
    static {
        try {
            ...
            Log.w(LOG_TAG, "Native library not found, native RGB -> YUV conversion may be unavailable.");
        }
    }
    ...
    public static void convertYUV420ToARGB8888(
        ...
        if (useNativeConversion) {
            try {
                ...
                Log.w(LOG_TAG, "Native YUV -> RGB implementation not found, falling back to Java implementation");
        ...
        for (int row = 0; row < height; row++) {
            int yRowStart = yRowStride * row;
            int uvRowStart = uvRowStride * (row >> 1);

            for (int column = 0; column < width; column++) {
                int yOffset = yRowStart + column;
                int uvOffset = uvRowStart + (column >> 1) * uvPixelStride;

                out[i++] =
                        YUV2RGB(
                                convertByteToInt(yData, yOffset),
                                convertByteToInt(uData, uvOffset),
                                convertByteToInt(vData, uvOffset));
            }
        }
    }
